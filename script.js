import {
  ...
  function saveTodos() {
    const todosToSave = todos.map(({ isNew, ...rest }) => rest);
    localStorage.setItem("todos", JSON.stringify(todosToSave));
  }
  function addTodo() {
    const text = todoInput.value.trim();
    if (!text) return;
    todos.push({ text, completed: false, isNew: true });
    saveTodos();
    renderTodos();
    todoInput.value = "";
    todoInput.focus();
...
function formatAIResponse(text) {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (listItemMatch) {
        ...
        if (line.trim()) {
          ...
          // Keep empty lines for spacing between paragraphs/blocks if needed
          // Or just skip adding anything for consecutive newlines
        }
      }
    }
    ...
    // to avoid extra <p> wrapping around them.
  ...
  function maybeEnableCalendar() {
    if (gapiInited && gisInited) {
      const token = gapi.client.getToken(); // Check if a token is ALREADY loaded (e.g., from a previous action in this session)
  
      if (token) {
        // If a token exists in memory, update UI and fetch data
        console.log("Token already exists in GAPI client memory.");
        updateSigninStatus(true);
        fetchAndDisplayCalendarEvents();
      } else {
        // --- THIS IS THE NEW PART ---
        // No token in memory, try to get one silently
    ...
    document.addEventListener("DOMContentLoaded", () => {
      ...
      if (
        ...
        gapiAuthorizeButton.onclick = () => {
          ...
          // Ensure gapiTokenClient is initialized before calling
          if (gapiTokenClient) {
